...
add_filter('peepso_navigation_profile', function($links) {
    if(isset($links['stream'])) { $links['stream']['icon'] = 'gcis gci-stream'; }
    return $links;
},9999);


// Event Listing option in elementor pro
...
//
// Wpforo share image url
//

function my_default_og_image( $image_url, $type ){
if( $type == 'og:image' && !$image_url){
return 'https://dev.dojoko.com/wp-content/uploads/2020/11/Icon-96x96-Desktop-Shortcut.png';
}
else{
return $image_url;
}
...
add_filter('wpforo_find_image_urls', 'my_default_og_image', 10, 2);
...
//	global $post;
//
//	if (! is_user_logged_in() && !in_array($post->ID, array(213,225,3232,223))) { 
//		wp_safe_redirect('http://dev.dojoko.com/login/');
//     exit();
//
//   }
//
...
//
add_filter('peepso_navigation_profile', function($links) {
    if(isset($links['forums'])) { $links['forums']['icon'] = 'gcis dojokoquill'; }
    return $links;
},9999);

//
// Profile Info Section in User Menu
//

add_filter('peepso_navigation_profile', function($navigation){

	$PeepSoProfile = PeepSoProfile::get_instance();
	$PeepSoUser = $PeepSoProfile->user;
	 $user_url = $PeepSoUser->get_username();
    
    $url = site_url();
   
    $navigation['cv'] = array(   // this key needs to be unique to your plugin
      'href' => $url.'/onboarding/?from=resume',          
      'label' => __('CV','CV'),
      'icon'  => 'gcis dojokoprofile',      
      'primary'           => TRUE,  // Main toolbar
      'secondary'         => TRUE, // Notification area
      'mobile-primary'    => TRUE,  // Mobile hamburger menu
      'mobile-secondary'  => TRUE, // Mobile notification area
      'widget'            => TRUE,  // Profile widget "community links"
    );
    return ($navigation);
});

//
// Event Search Widget by RJ
//
...
__('Event Search RJ', 'wpb_widget_domain'),

// Widget description
array( 'description' => __( 'Event Widget created by RJ', 'wpb_widget_domain' ), )
);
}
...
<form method="GET" action="/events/">
  <p>
		<input type="text" id="search_keywords" placeholder="Keywords" style="margin-bottom:15px;" name="search_keywords" />
 </p>

	<p>
    <input type="text" id="search_location" name="search_location" placeholder="Location" style="margin-bottom:15px;" />
  </p>
...
<p>
		<select id="search_type" name="search_type">
			<option value="" disabled selected hidden>Event Types</option>
...
// Rishabh Code Ends for Event Search Widget
...
{
	return str_replace( 'wp-login', 'turk182', $default );
}
