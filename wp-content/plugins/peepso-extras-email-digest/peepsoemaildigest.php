...
class PeepSoEmailDigest
{
    ...
    const ICON = 'https://www.peepso.com/wp-content/plugins/peepso.com-checkout/assets/icons/emaildigest_icon.svg';
	...
	function cron_email_digest_generate($schedule_type)
	{
		...
		if (PeepSo::get_option('email_digest_enable') === 1) {
			...
			new PeepSoError('ED User query : ' . $user_query->request);
			...
			if (count($user_results) > 0) {
				...
				foreach ($user_results as $user_item) {
					new PeepSoError('[ED] Batch: ' . print_r($user_item, true));
