...
//Channel Tab by rishabh
	add_action('peepso_group_segment_channels', array(&$this, 'filter_group_segment_channels'));
    add_action('peepso_group_segment_messages', array(&$this, 'filter_group_segment_messages'));
    ...
    public function filter_group_segment_channels($args)
    {
        $PeepSoUrlSegments = PeepSoUrlSegments::get_instance();
        if($tab = $PeepSoUrlSegments->get(3)) {
            PeepSoTemplate::exec_template('groups', 'group-channels-'.$tab, $args);
            return;
        }

        echo PeepSoTemplate::exec_template('groups', 'group-channels', $args);
    }
    
     public function filter_group_segment_messages($args)
    {
        $PeepSoUrlSegments = PeepSoUrlSegments::get_instance();
        if($tab = $PeepSoUrlSegments->get(3)) {
            PeepSoTemplate::exec_template('groups', 'group-messages-'.$tab, $args);
            return;
        }

        echo PeepSoTemplate::exec_template('groups', 'group-messages', $args);
    }
