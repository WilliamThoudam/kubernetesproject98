...
class PeepSoGroup
{
	...
	public function __construct($id = NULL, $data = NULL)
	{
		...
		if( NULL == $id && is_array($data) ) {
			...
			$this->id = $id = $this->create($data['name'], $data['group_parent_id']);
	...
	public function move_cover_file($src_file)
	{
		...
		if($enable_aws_s3)
		{
			...
			$pho_token = $this->upload_to_amazon_s3($dest_file, $filename);
			if(NULL !== $pho_token)
			{
				update_post_meta($this->id, 'group_cover_photo', $pho_token);
	...
	public function upload_to_amazon_s3($filepath, $filename)
	{
