...
class PeepSoGroupUser
{
    ...
    public function get_member_actions()
    {
        ...
        if($this->is_pending_user) {
        ...
        } elseif($this->can('join_request') && ($par_id == 0) ) {

            // Can't join, but can request
            // PeepSoGroupsAjax::join_request
            $actions[] = array(
            'action' 		=> 'join_request',
            'label'			=> __('Request To Join Group', 'groupso'),
            );

        } elseif($this->can('join_request') && ($par_id > 0) ) {

            $PeepSoGroupUser = new PeepSoGroupUser($par_id);
            ...
            if($PeepSoGroupUser->is_member)
            {
                ...
                $actions[] = array(
                'action' 		=> 'join_request',
                'label'			=> __('Request To Join Channel', 'groupso'),
                );
            }
            else
            {
                ...
                $actions[] = array(
                'action' 		=> NULL,
                'label'			=> __('Not a member of Parent Group', 'groupso'),
                'message'		=> __('Please Join Parent Group to be a part of this channel', 'groupso'),
                );
            }
