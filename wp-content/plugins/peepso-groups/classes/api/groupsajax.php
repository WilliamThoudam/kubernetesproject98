...
class PeepSoGroupsAjax extends PeepSoAjaxCallback
{
	...
	public function search(PeepSoAjaxResponse $resp)
	{
        ...
        $gid = $this->_input->int('group_parent_id', 1);
		...
		$groups = $PeepSoGroups->get_groups($offset, $limit + 1, $order_by, $order, $query, $user_id, $category, $gid);
        ...
        if (count($groups) > 0 || $page > 1) {
			...
			if($user_id) {
              ...
              if($gid > 0)
              {
               $msg_text = 'No channels found'; }
              else
              {    $msg_text = 'No groups found'; }
              $message = $msg_text;
