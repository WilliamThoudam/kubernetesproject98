...
class PeepSoGroupUserAjax extends PeepSoGroupAjaxAbstract
{
	...
	public function join(PeepSoAjaxResponse $resp)
	{
        ...
        // RJ : Adding user to a message chat for new Channel

				$post_parent_id = wp_get_post_parent_id($this->_group_id);
				if($post_parent_id != 0)
				{
						$msg_id_join = $this->_group_id + 1;
            $peepso_participants = new PeepSoMessageParticipants();
            $peepso_participants->add_participant($msg_id_join, get_current_user_id());
						$peepso_participants->set_group($msg_id_join);
				}
	...
	public function leave(PeepSoAjaxResponse $resp)
	{
        ...
        $parent_id = wp_get_post_parent_id($this->_group_id);

        if($parent_id > 0)
        {
            $allposts = $wpdb->get_results( $wpdb->prepare("SELECT ID FROM ".$wpdb->posts." where `post_type` = 'peepso-message' AND `post_content_filtered` = ".$this->_group_id));

            foreach ($allposts as $singlepost) { $msg_id = $singlepost->ID; }

            $peepso_messages_participants = new PeepSoMessageParticipants();
            $peepso_messages_participants->remove_participant(get_current_user_id(),$msg_id);
        ...
        {
            $id_channels = $wpdb->get_results( $wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE `post_type` = 'peepso-group' AND `post_parent` =".$this->_group_id));
            ...
            foreach ( $id_channels as $id_channel)
            {

				$chn_id = $id_channel->ID;

				$allposts = $wpdb->get_results( $wpdb->prepare("SELECT ID FROM ".$wpdb->posts." where `post_type` = 'peepso-message' AND `post_content_filtered` = ".$chn_id));

				foreach ($allposts as $singlepost)
				{
					$msg_id = $singlepost->ID;
				}

				$peepso_messages_participants = new PeepSoMessageParticipants();
				$peepso_messages_participants->remove_participant(get_current_user_id(),$msg_id);

				$this->_model= new PeepSoGroupUser($chn_id, $this->_user_id);

				$this->_model->member_leave();
				$resp->success(1);

				do_action('peepso_action_group_user_delete', $chn_id, $this->_user_id);

				$actions = array_merge($this->_model->get_member_actions(), $actions);
				$resp->set('member_actions', $actions);

				$PeepSoGroupUsers = new PeepSoGroupUsers($chn_id);
				$resp->set('member_count', $PeepSoGroupUsers->update_members_count());
				$resp->set('pending_admin_member_count', $PeepSoGroupUsers->update_members_count('pending_admin'));
	...
	public function passive_modify(PeepSoAjaxResponse $resp)
	{
			...
			if(isset($action_user_join_request_accept))
			{
			    do_action('peepso_action_group_user_join_request_accept', $this->_group_id, $this->_passive_user_id);
				 	...
				 	if($parent_id != 0)
					{
						$msg_id_join = $this->_group_id + 1;
						$peepso_participants = new PeepSoMessageParticipants();
						$peepso_participants->add_participant($msg_id_join, $this->_passive_user_id);
						$peepso_participants->set_group($msg_id_join);
					}
		...
		if(isset($action_user_delete))
        {
			...
			if($parent_id > 0)
			{
				// RJ : Admin is removing from Channel, so removing him from Message Chat.
				...
				$peepso_messages_participants = new PeepSoMessageParticipants();
				$peepso_messages_participants->remove_participant($this->_passive_user_id, $msg_id);
			...
			{
				// RJ : Admin removing user from Parent Grp, Its associated channels and messages chat

                $id_channels = $wpdb->get_results( $wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE `post_type` = 'peepso-group' AND `post_parent` =".$this->_group_id));

				foreach ( $id_channels as $id_channel)
				{
                    // RJ : Removing user channels and messages chat
                    ...
                    $this->_model= new PeepSoGroupUser($chn_id, $this->_passive_user_id);

                    $this->_model->member_leave();
                    $resp->success(1);

                    do_action('peepso_action_group_user_delete', $chn_id, $this->_passive_user_id);
                ...
                // RJ : Removing user from Parent Grp
