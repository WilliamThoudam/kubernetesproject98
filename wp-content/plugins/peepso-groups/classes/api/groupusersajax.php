...
class PeepSoGroupUsersAjax extends PeepSoAjaxCallback
{
	...
	public function search_to_invite(PeepSoAjaxResponse $resp, $keys = NULL)
	{
		$users = array();
		$gid = $this->_group_id;
		$parent_id = wp_get_post_parent_id($gid);
		if($parent_id > 0)
		{
			$group_user = new PeepSoGroupUser($parent_id);
		}
		else
		{
			$group_user = new PeepSoGroupUser($gid);	
		}
		...
		if ($group_user->can('invite')) {
			...
			if($parent_id > 0)
			{
				$parent_response = new PeepSoGroupUsers($parent_id);
				$parent_users = $parent_response->get_members($args, $query, $order_by, $order, $offset, $limit);


				$channel_response = new PeepSoGroupUsers($gid);
				$channel_users = $channel_response->get_members($args, $query, $order_by, $order, $offset, $limit);

				foreach($channel_users as $channel_user)
				{
					$channel_members[] = $channel_user->user_id;
				}
				...
				if(count($parent_users)) 
				{
					...
					{
						if(!in_array($user->user_id, $channel_members))
						{
							$member = PeepSoGroupAjaxAbstract::format_response($user, PeepSoGroupAjaxAbstract::parse_keys('groupuser', $keys), $parent_id);
					...
					if(!empty($members_response))
					{
						$resp->success(TRUE);
						$resp->set('users', $members_response);
					}
					else
					{
						$resp->success(FALSE);
