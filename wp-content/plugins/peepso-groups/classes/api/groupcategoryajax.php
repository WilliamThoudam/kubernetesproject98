...
class PeepSoGroupCategoryAjax extends PeepSoAjaxCallback
{
    ...
    public function avatar_crop(PeepSoAjaxResponse $resp)
    {
        ...
        if (wp_verify_nonce($this->_input->value('_wpnonce','',FALSE), 'group-category-avatar') && PeepSo::is_admin()) {
            ...
            $src_file = $PeepSoGroupCategory->get_image_dir() . $avatar_hash . 'avatar-orig' . ($tmp ? '-tmp' : '') . '.jpg';
            $dest_file = $PeepSoGroupCategory->get_image_dir() . $avatar_hash  . 'avatar-full' . ($tmp ? '-tmp' : ($preview ? '-preview' : '')) . '.jpg';
            ...
            $dest_file = $PeepSoGroupCategory->get_image_dir() . $avatar_hash . 'avatar' . ($tmp ? '-tmp' : ($preview ? '-preview' : '')) . '.jpg';
            ...
            if ($tmp) {
                ...
                $image_url = str_replace('.jpg', '-preview.jpg', $PeepSoGroupCategory->get_avatar_url());
            } else {
                $image_url = $PeepSoGroupCategory->get_avatar_url();
            }
