...
class PeepSoGroupAjax extends PeepSoGroupAjaxAbstract
{
    ...
    public function create(PeepSoAjaxResponse $resp)
    {
        ...
        $group_data = array(
            ...
            'group_parent_id' => $this->_input->value('group_parent_id', '', FALSE), // SQL safe,
            'owner_id'		=> get_current_user_id(),
            'meta'			=> $this->_input->value('meta', array(), FALSE), // SQL safe,
        );


        if(PeepSo::get_option('groups_categories_enabled', FALSE) && ($group_data['group_parent_id'] == 0) )
        {
        ...
        if($group_data['group_parent_id'] != 0)
        {
            // Channel-Messaging Intiation
            $peepso_participants = new PeepSoMessageParticipants();
            $subject = 'Channel Message';
            $message = htmlspecialchars('Welcome to Channel Messaging');
            ...
            // Temp Check -- Need Retesting
            $peepso_participants->set_group($msg_id);

            $my_post['ID'] = $msg_id;
            $my_post['post_author'] = get_current_user_id();
            $my_post['post_content_filtered'] = $group->id;

            wp_update_post($my_post, true);
            update_post_meta($group->id, 'peepso_group_is_invitable', 0);

            if(isset($group))
            {
                $response = $peepso_participants->add_participant($msg_id, $creator_id);
                if($response == 1)
                {
                    $peepso_recipients = new PeepSoMessageRecipients();
                    $peepso_recipients->add_message($msg_id);
                }
            }
        }
    ...
    public function avatar_crop(PeepSoAjaxResponse $resp)
    {
        ...
        if (wp_verify_nonce($this->_input->value('_wpnonce','',FALSE), 'group-avatar') && $group_user->can('manage_content')) {
            ...
            $dest_file = $group->get_image_dir() . $avatar_hash  . 'avatar-full' . ($tmp ? '-tmp' : ($preview ? '-preview' : '')) . '.jpg';
            ...
            $dest_file = $group->get_image_dir() . $avatar_hash . 'avatar' . ($tmp ? '-tmp' : ($preview ? '-preview' : '')) . '.jpg';
            ...
            if ($tmp) {
                ...
                $image_url = str_replace('.jpg', '-preview.jpg', $group->get_avatar_url());
            } else {
                $image_url = $group->get_avatar_url();
            }
