<script type="text/javascript" src="/wp-content/themes/peepso-theme-gecko/assets/js/multiple_email_input.js"></script>
<?php
    ...
    $PeepSoUrlSegments = PeepSoUrlSegments::get_instance();
        ...
        if ($PeepSoUrlSegments->get(2) == 'messages')
        {
            $is_message = 0;
        }
...
<div class="breadcrumb">
    ...
    $parent_id = wp_get_post_parent_id($group->id);

    if($parent_id != 0)
    {
        $par_grp = new PeepSoGroup($parent_id);
        $par_name = $par_grp->get('name');
        $par_url = $par_grp->get_url();
		$channel_url = $par_grp->get_url(). 'channels/&'. $parent_id;

        echo '<a href="'.home_url().'/groups" rel="nofollow"><i class="gcis dojokousers"></i></a>';
        echo "&nbsp;&nbsp;&#187;&nbsp;&nbsp;";
        echo '<a href="'.$par_url.'" rel="nofollow">'.$par_name.'</a>';
        echo "&nbsp;&nbsp;&#187;&nbsp;&nbsp;";
		echo '<a href="'.$channel_url.'" rel="nofollow"><i class="dojokoChannelicon"></i></a>';
        echo "&nbsp;&nbsp;&#187;&nbsp;&nbsp;";
...
<div class="ps-focus ps-focus--group ps-group__profile-focus ps-js-focus ps-js-focus--group ps-js-group-header">
      ...
      <div class="ps-focus__cover <?php if($is_message == 0){ echo ("is_message_channel"); } ?> ps-js-cover">
		<div class="ps-focus__cover-image ps-js-cover-wrapper">
			<img class="ps-js-cover-image" src="<?php echo $PeepSoGroup->get_cover_url(); ?>"
				...
				style="<?php echo $PeepSoGroup->cover_photo_position(); ?>; opacity: 0; transition: opacity 0.5s" />
		</div>
	...
	<div class="ps-focus__footer ps-group__profile-focus-footer">
		<div class="ps-focus__info ?> ">
			<div class="ps-focus__title">
                ...
                <div class="ps-group__details-item ps-focus__desc-toggle ps-tip ps-tip--absolute ps-tip--inline ps-tip--bottom ps-js-focus-box-toggle" aria-label="<?php if($parent_id != 0){echo __('About Channel','groupso');}else{echo __('About Group', 'groupso');}?>"><i class="gcis gci-info-circle"></i>
				    </div>
				...
				<div class="ps-focus__detail">
					<?php if($PeepSoGroupUser->can('manage_group') && strlen($group_segment) && 'settings' == $group_segment) { ?>
						<div class="ps-group__profile-privacy ps-dropdown ps-dropdown--privacy ps-js-dropdown ps-js-privacy ps-js-privacy--<?php echo $group->id; ?>">
							<a href="javascript:" data-value="" class="cus_group_heading_btns ps-btn ps-btn--sm ps-btn--dropdown ps-dropdown__toggle ps-js-dropdown-toggle">
					...
					<?php } else { ?>
						<span class="cus_group_heading_btns ps-btn ps-btn--sm ps-btn--app ps-tip ps-tip--bottom ps-tip--md ps-tip--arrow ps-tip--left" aria-label="<?php echo $group->privacy['desc'];?>">
							<i class="<?php echo $group->privacy['icon'];?>"></i><?php if($parent_id != 0){echo sprintf(__('%s Channel','groupso'), $group->privacy['name']);}else{echo sprintf(__('%s Group','groupso'), $group->privacy['name']);}?>
						</span>
					<?php } ?>
				</div>
		...
		<div class=" ps-focus__menu ps-js-focus__menu" id="group_wrapper_id">
            ...
            //New tab for Messaging in channels by Rishabh
                $msg_id = $group->id + 1;

                  if(($parent_id != 0) && ($PeepSoGroupUser->is_member) ) {
                $segments[0][] = array(
					'href' => 'messages/&'.$msg_id,
					'title'=> 'Messages',
					'icon' => 'dojokoenvelop',
					'id'=> 'cus_msg_tab',
				);
    ...
    //New tab for channels by Rishabh
                ...
                $segments[0][] = array(
					...
					'title'=> 'Channels',
					'icon' => 'dojokoChannelicon',
					'id' => 'cus_group_channel_tab',
				);
                }
				...
				foreach($segments as $segment_group) {
					foreach($segment_group as $segment) {
						...
						if($can_access) {
						?>
						<a <?php if($segment['id'] == 'cus_group_channel_tab') echo 'id="'.$segment['id'].'"'; ?> <?php if($segment['id'] == 'cus_msg_tab') echo 'id="'.$segment['id'].'"'; ?> class="ps-focus__menu-item ps-js-item <?php echo($segment['href'] == $group_segment) ? 'ps-focus__menu-item--active':'';?>" href="<?php echo $href; ?>">
...
jQuery(function() {
    ...
    jQuery(function() {
        ...
        jQuery('#to_emails').multiple_emails({
            position: "top"
        });
        //OR $('#to_emails').multiple_emails("Bootstrap");

        //Shows the value of the input device, which is in JSON format
        jQuery('#current_emailsBS').text(jQuery('#to_emails').val());
        // $('#to_emails').change( function(){
