...
import peepsodata, { group_category as categoryData } from 'peepsodata';

const USER_ID = +peepsodata.currentuserid;
const UPLOAD_MAXSIZE = +peepsodata.upload_size;

const CATEGORY_ID = +categoryData.id;
const IMG_COVER_DEFAULT = categoryData.img_cover_default;
const COVER_NONCE = categoryData.cover_nonce;
const COVER_UPLOAD_URL = `${peepsodata.ajaxurl_legacy}groupcategoryajax.cover_upload?cover`;
const COVER_TEXT_ERROR_FILETYPE = categoryData.text_error_filetype;
const COVER_TEXT_ERROR_FILESIZE = categoryData.text_error_filesize;

let hasCover = +categoryData.has_cover;

$(function () {
	let $container = $('.ps-js-focus--group-category .ps-js-cover');
	if (!$container.length) {
		return;
	}
	...
	function uploadInit() {
		let name = 'group-category-cover-upload',
			accept = 'image/*',
		...
		$file.psFileupload({
			formData: { category_id: CATEGORY_ID, _wpnonce: COVER_NONCE },
			dataType: 'json',
			...
			add(e, data) {
				...
				if (json.success) {
					...
					hooks.doAction('group_cover_updated', CATEGORY_ID, imgCover);
				} else if (json.errors) {
					alert(json.errors);
				}
			}
	...
	function remove(e) {
		...
		let popup = dialog(categoryData.template_cover_remove).show();
		popup.$el.on('click', '.ps-js-cancel', () => popup.hide());
		popup.$el.on('click', '.ps-js-submit', () => {
			popup.hide();
			$coverImage.css('opacity', 0.5);

			let params = { category_id: CATEGORY_ID, _wpnonce: COVER_NONCE };
			ajax.post('groupcategoryajax.cover_delete', params).then(json => {
				if (json.success) {
					$remove.hide();
					$reposition.hide();
					hasCover = false;
					...
					hooks.doAction('group_cover_updated', CATEGORY_ID, IMG_COVER_DEFAULT);
				}
			});
		});
	}
	...
	function repositionConfirm(e) {
		...
		ajax.post('groupcategoryajax.cover_reposition', params); // Optimistic call.
	}
	...
	let _docWidth;
	hooks.addAction('browser_resize', 'group_cover', dimension => {
	...
	hooks.addAction('group_cover_updated', 'page_group_category', (id, imageUrl) => {
		if (+id === CATEGORY_ID) {
			if (imageUrl.indexOf(`/group-categories/${id}/`) > -1) {
				// Custom cover.
