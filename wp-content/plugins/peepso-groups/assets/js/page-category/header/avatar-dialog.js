...
const IMG_AVATAR_DEFAULT = categoryData.img_avatar_default;
const AVATAR_NONCE = categoryData.avatar_nonce;
const AVATAR_UPLOAD_URL = `${peepsodata.ajaxurl_legacy}groupcategoryajax.avatar_upload`;
const AVATAR_TEXT_ERROR_FILETYPE = categoryData.text_error_filetype;
const AVATAR_TEXT_ERROR_FILESIZE = categoryData.text_error_filesize;

let hasAvatar = +categoryData.has_avatar,
	imgAvatar,
	imgAvatarOriginal,
	popup,
	$container,
...
function init() {
	...
	update.apply(
		null,
		hasAvatar ? [categoryData.img_avatar, categoryData.img_avatar_original] : []
	);
...
function uploadInit() {
	let name = 'group-category-avatar-upload',
		accept = 'image/*',
	...
	$file.psFileupload({
		formData: { category_id: CATEGORY_ID, _wpnonce: AVATAR_NONCE },
		dataType: 'json',
...
function remove(e) {
	...
	let params = { category_id: CATEGORY_ID, _wpnonce: AVATAR_NONCE };
	ajax.post('groupcategoryajax.avatar_delete', params).then(json => {
		if (json.success) {
			update(null);
			...
			hooks.doAction('group_category_avatar_updated', CATEGORY_ID, IMG_AVATAR_DEFAULT);
		}
	});
}
...
function finalize(e) {
	...
	if (imgIsTemporary) {
		...
		finalizing = true;
		ajax.post('groupcategoryajax.avatar_confirm', params)
			.then(json => {
				if (json.success) {
					let avatar = json.data && json.data.image_url;
					...
					imgIsTemporary = false;
					$done.attr('disabled', 'disabled');
					hooks.doAction('group_category_avatar_updated', CATEGORY_ID, avatar);
				}
			})
			.always(function () {
				finalizing = false;
		...
		let params = $.extend(
			imgCropData,
			{ u: CATEGORY_ID, _wpnonce: AVATAR_NONCE },
			{ tmp: '', preview: '' }
		);

		finalizing = true;
		ajax.post('groupcategoryajax.avatar_crop', params)
			.then(json => {
				if (json.success) {
					let avatar = json.data && json.data.image_url;
...
function cropConfirm(e) {
	...
	let params = $.extend(cropData, { u: CATEGORY_ID, _wpnonce: AVATAR_NONCE });
	ajax.post('groupcategoryajax.avatar_crop', params).then(json => {
		if (json.success) {
...
export default function () {
	if (!popup) {
		popup = dialog(categoryData.template_avatar, { wide: true, destroyOnClose: false }).show();
		init();
	}
