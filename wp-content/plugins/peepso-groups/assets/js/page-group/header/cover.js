...
import { ajaxurl_legacy, currentuserid as USER_ID, group as groupData } from 'peepsodata';

const GROUP_ID = +groupData.id;
const MODULE_ID = +groupData.module_id;
const IMG_COVER_DEFAULT = groupData.img_cover_default;
const COVER_NONCE = groupData.cover_nonce;
const COVER_UPLOAD_URL = `${ajaxurl_legacy}groupajax.cover_upload?cover`;
const COVER_UPLOAD_MAXSIZE = +peepsodata.upload_size;
const COVER_TEXT_ERROR_FILETYPE = groupData.text_cover_error_filetype;
const COVER_TEXT_ERROR_FILESIZE = groupData.text_cover_error_filesize;

let hasCover = +groupData.has_cover;

$(function () {
	let $container = $('.ps-js-focus--group .ps-js-cover');
	if (!$container.length) {
		return;
	}
	...
	function uploadInit() {
		...
		$file.psFileupload({
			formData: { group_id: GROUP_ID, module_id: MODULE_ID, _wpnonce: COVER_NONCE },
			dataType: 'json',
			...
			add(e, data) {
				...
				if (json.success) {
					...
					hooks.doAction('group_cover_updated', GROUP_ID, imgCover);
				} else if (json.errors) {
					alert(json.errors);
				}
			}
	...
	function remove(e) {
		...
		let popup = dialog(groupData.template_cover_remove).show();
		popup.$el.on('click', '.ps-js-cancel', () => popup.hide());
		popup.$el.on('click', '.ps-js-submit', () => {
			popup.hide();
			$coverImage.css('opacity', 0.5);

			let params = { group_id: GROUP_ID, _wpnonce: COVER_NONCE };
			ajax.post('groupajax.cover_delete', params).then(json => {
				if (json.success) {
					$remove.hide();
					$reposition.hide();
					hasCover = false;
					...
					hooks.doAction('group_cover_updated', GROUP_ID, IMG_COVER_DEFAULT);
				}
			});
		});
	}
	...
	function repositionConfirm(e) {
		...
		ajax.post('groupajax.cover_reposition', params); // Optimistic call.
	}
	...
	hooks.addAction('group_cover_updated', 'page_group', (id, imageUrl) => {
		if (+id === GROUP_ID) {
			if (imageUrl.indexOf(`/groups/${id}/`) > -1) {
				// Custom cover.
