...
class PeepSoGroupsPlugin
{
    ...
    public function init()
    {
        ...
        if (is_admin()) {
            ...
            add_action('peepso_groups_new_post', function($group_id, $post_id) {
                ...
                if ($post->post_type != 'peepso-message')
                {
    ...
    public function filter_post_clauses($clauses, $user_id = NULL) {
        ...
        if (!is_null($user_id) && (strpos($clauses['where'], PeepSoActivityStream::CPT_COMMENT) === false)) {
            ...
            if( in_array($this->input->value('context', '', FALSE), array('group-category') ) ) {
                ...
                $clauses['where'] .= $wpdb->prepare(" AND `pm`.`meta_value` IS NOT NULL AND `grp`.`ID` IN (" . implode(',',$group_ids) . ")");
            }
        ...
        if(!empty($group_id) && (trim($this->url_segments->_shortcode) != 'peepso_groups') && ($module_id !== self::MODULE_ID || $force_as_group_post)) {
            ...
            if(FALSE !== $group) {
                ...
                if($is_join_notification) {
                    ...
                    $title = sprintf(
                        '<a class="ps-post__author" href="%s" data-hover-card="%d">%s</a> '. $action_title . ' <a class="ps-post__subtitle" href="%s" title="%s"><i class="gcis gci-users"></i>%s</a><span class="ps-post__title-desc ps-stream-action-title">' . $action . '</span> ',
                        $author->get_profileurl(), $author->get_id(), $before_fullname . $author->get_fullname() . $after_fullname,
                        $group->get_url(), $meta, $group->get('name')
                    );
                } else {
                    $title = sprintf(
                        '<a class="ps-post__author" href="%s" data-hover-card="%d">%s</a><i class="gcis gci-angle-right"></i><a class="ps-post__subtitle" href="%s" title="%s"><i class="gcis gci-users"></i>%s</a><span class="ps-post__title-desc ps-stream-action-title">' . $action . '</span> ',
                        $author->get_profileurl(), $author->get_id(), $before_fullname . $author->get_fullname() . $after_fullname,
                        $group->get_url(), $meta, $group->get('name')
                    );
                }
    ...
    public function set_post_status($aPostData) {
        ...
        if(0 !== $group_id) {
            ...
            if(is_array($aPostData) && (0 === intval(PeepSo::get_option('photos_groups_enable_post_updates_group_avatar',1)) || FALSE === $group->published)) {
                $aPostData['post_status'] = 'pending';
            }

            if(is_array($aPostData) && (0 === intval(PeepSo::get_option('photos_groups_enable_post_updates_group_cover',1))  || FALSE === $group->published)) {
                $aPostData['post_status'] = 'pending';
            }
        }
    ...
    public function action_group_user_join($group_id, $user_id)
    {
        ...
        $par_id = wp_get_post_parent_id($group_id);

        if($par_id > 0)
        {
            $msg_txt = 'joined your channel';
        }
        else
        {
            $msg_txt = 'joined your group';
        }
        ...
        if(!$PeepSoGroup->is_join_muted) {
            ...
            foreach ($owners as $PeepSoGroupUser) {
                $PeepSoNotifications->add_notification($user_id, $PeepSoGroupUser->user_id, __($msg_txt, 'groupso'), 'groups_user_join', self::MODULE_ID, $group_id);
            }
        }
    ...
    public function action_group_user_invitation_accept(PeepSoGroupUser $PeepSoGroupUser)
    {
        ...
        if($par_id > 0)
        {
            $msg_txt = 'accepted your channel invitation';
        }
        else
        {
            $msg_txt = 'accepted your group invitation';
        }

        if($PeepSoGroupUser->invited_by_id) {
            $PeepSoNotifications = new PeepSoNotifications();
            $PeepSoNotifications->add_notification(get_current_user_id(), $PeepSoGroupUser->invited_by_id, __($msg_txt, 'groupso'), 'groups_user_invitation_accept', self::MODULE_ID, $PeepSoGroupUser->group_id);
        }
    }
    ...
    public function action_group_user_join_request_send($group_id) {
        ...
        foreach($owners as $PeepSoGroupUser) {
            $PeepSoNotifications->add_notification(get_current_user_id(), $PeepSoGroupUser->user_id, $message.$msg_txt, 'groups_user_join_request_send', self::MODULE_ID, $group_id);
        }
    }
    ...
    private function notification_message_user_join_request(PeepSoGroupUsers $PeepSoGroupUsers, $user_id)
    {
        ...
        $message .= __('requested to join your ', 'groupso');
    ...
    public function action_group_user_join_request_accept($group_id, $user_id)
    {
        ...
        if($par_id > 0)
        {
            $msg_txt = 'accepted you as a channel member';
        }
        else
        {
            $msg_txt = 'accepted you as a group member';
        }

        $this->notification_update_user_join_request($group_id, $user_id);
        $PeepSoNotifications = new PeepSoNotifications();
        $PeepSoNotifications->add_notification(get_current_user_id(), $user_id, __($msg_txt, 'groupso'), 'groups_user_join_request_accept', self::MODULE_ID, $group_id);
    ...
    public function action_group_rename($group_id, $user_id) {
        ...
        if($par_id > 0)
        {
            $msg_txt = 'renamed a channel you\'re a member of';
        }
        else
        {
            $msg_txt = 'renamed a group you\'re a member of';
        }
        ...
        if(count($list_members) > 0) {
            ...
            foreach($list_members as $groupuser) {
                if ($groupuser->user_id != $user_id) {
                    $notif->add_notification($user_id, $groupuser->user_id, __($msg_txt, 'groupso'), 'groups_rename', self::MODULE_ID, $group_id);
                }
            }
        }
    }
    ...
    public function action_group_privacy_change($group_id, $user_id) {
        ...
        if($par_id > 0)
        {
            $msg_txt = 'changed privacy of a channel ';
        }
        else
        {
            $msg_txt = 'changed privacy of a group ';
        }
            ...
            foreach($list_members  as $groupuser) {
                if ($groupuser->user_id != $user_id) {
                    $notif->add_notification(
                        $user_id,
                        $groupuser->user_id,
                        sprintf(__($msg_txt.' to %s', 'groupso'), $group->privacy['notif']),
        ...
        if($par_id != 0)
        {
            ...
            $data = array(
                ...
                'lang' => array(
                    ...
                    'name_change_confirmation' => __('Are you sure you want to change the channel name?','groupso') .'<br>' . __('All channel members will be notified.','groupso'),
                    'slug_change_confirmation' => __('Are you sure you want to change the channel URL?','groupso') .'<br>' . __('All channel members will be notified.','groupso'),
                    'privacy_change_confirmation' => __('Are you sure you want to change the channel privacy?','groupso') .'<br>' . __('All channel members will be notified.','groupso'),
        ...
        if ($group_id) {
            ...
            add_filter('peepso_data', function( $data ) {
                ...
                // Or from the URL.
                    ...
                    $is_front_page = 'page' === get_option('show_on_front') && $post->ID == get_option('page_on_front');
                    $segment_part = $is_front_page ? 0 : 1;
                    $group_id = $this->url_segments->get($segment_part);
                    if ($group_id) {
                        $group = new PeepSoGroup($group_id);
                ...
                if ($group_id) {
                    $group_data = array(
                        ...
                        'text_error_filetype' => __('The file type you uploaded is not allowed. Only JPEG/PNG allowed.', 'groupso'),
