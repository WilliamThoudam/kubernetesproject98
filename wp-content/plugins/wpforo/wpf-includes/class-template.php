                           ...
                           <?php if( !$values ) : ?>
                              ...
                              <?php if(WPF()->perm->forum_can('p', $forumid) || WPF()->perm->forum_can('op', $forumid)) : ?>
                                  <!-- <input id="wpf_t_private_<?php echo $uniqid ?>" name="topic[private]" type="checkbox" value="1">&nbsp;&nbsp;
                                   ...
                                   <label for="wpf_t_private_<?php echo $uniqid ?>" style="padding-bottom:2px; cursor: pointer;" title="<?php wpforo_phrase('Only Admins and Moderators can see your private topics.'); ?>"><?php wpforo_phrase('Test'); ?>&nbsp;</label> -->
                               <?php endif; ?>
                            <?php endif ?>
	...
	*/
	function reply_form($topic = array(), $values = array()){
		...
		<div class="wpf-form-wrapper wpfel-<?php echo $layout ?>" <?php echo $style ?>>
            <?php if( !$values ): ?>
                <p class="wpf-reply-form-title"><?php echo apply_filters( 'wpforo_reply_form_head', wpforo_phrase('Leave an Answer', false), $topic ) ?></p>
            <?php endif ?>
				...
				<form enctype="multipart/form-data" method="POST" class="wpforoeditor <?php echo (!$values ? 'wpforo-main-form' : '') ?>" data-bodyminlength="<?php echo WPF()->post->options['post_body_min_length'] ?>" data-bodymaxlength="<?php echo WPF()->post->options['post_body_max_length'] ?>">
                    ...
                    <div class="wpf-buttons-wrap">
						<?php if($values) : ?>
                            ...
                            <input type="submit" class="wpf-button" value="<?php wpforo_phrase('Save') ?>" title="Ctrl+Enter">
						<?php else : ?>
                            <input type="submit" class="wpf-button" value="<?php $layout === 3 ? wpforo_phrase('Answer') : wpforo_phrase('Submit') ?>" title="Ctrl+Enter">
						<?php endif ?>
                    </div>
	 ...
	 * @param bool $echo
	 *
	 * $buttons = array( 'Answer', 'answer', 'comment', 'quote', 'like', 'report', 'sticky', 'close', 'edit', 'delete', 'link' );
	 *
	 * @return string
	 */
