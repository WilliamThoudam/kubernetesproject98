...
class wpForo_Widget_search extends WP_Widget {
	...
	public function widget( $args, $instance ) {
        ...
        <form action="<?php echo wpforo_home_url() ?>" method="get">
            ...
            <input type="text" placeholder="<?php wpforo_phrase('Start typing to search...') ?>" name="wpfs" class="wpfw-70" value="<?php echo isset($_GET['wpfs']) ? esc_attr(sanitize_text_field($_GET['wpfs'])) : '' ?>" ><button type="submit" class="wpfw-20"><i class="gcis gci-search"></i></button>
        </form>
		<?php
		echo '</div></div>';
...
//	    wp_enqueue_script('wpforo-widgets-js');
        ...
        if( !empty( $tags ) ){
            ...
            foreach( $tags as $tag ){
                ...
                echo '<li><a class="cus_tag_topic" href="' . esc_url( wpforo_home_url() . '?wpfin=tag&wpfs=' . $tag['tag'] ) . '" title="' . esc_attr($tag['tag']) . '">' . wpforo_text($tag['tag'], 25, false) . '</a>' . $topic_count . '</li>';
            }
            echo '</ul>';
            if( $instance['count'] < $items_count ){
                echo '<div class="wpf-all-tags"><a href="' . esc_url(wpforo_home_url(wpforo_get_template_slug('tags'))) . '">'. sprintf( wpforo_phrase('See more', false), $items_count ) . '</a></div>';
            }
         } else {
