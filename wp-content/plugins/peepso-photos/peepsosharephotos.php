...
class PeepSoSharePhotos
{
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_script('peepso-photos-postbox',
            PeepSo::get_asset('js/postbox.min.js', __FILE__),
    ...
    public function get_photo_group($objects, $photo_id)
    {
        ...
        if ( $owner_id && $album_id ) { // album photo set
            $photos = $this->_photos_model->get_user_photos_by_album($owner_id, $album_id, 0, 0, $sort ? $sort : 'desc', $module_id);
        } else if ( $owner_id && $module_id == 0) { // user photo set
            $photos = $this->_photos_model->get_user_photos($owner_id, 0, 0, $sort ? $sort : 'desc', $module_id);
        } else { // post (activity) photo set
            $photos = $this->_photos_model->get_post_photos($photo->pho_post_id, $sort ? $sort : 'asc');
        }
...
//        $items = array_merge($items_photos, $items_avatar, $items_cover, $items_album);
    ...
    /**
     * Add photo code to the notification link.
     *
     * @param string $link
     * @param array  $data
     * @return string
    ...
    public function modify_notification_link($link, $data) {
        // Only handle like and comment notifications.
        $type = $data['not_type'];
        if ( !in_array($type, array('user_comment', 'like_post')) ) {
            return $link;
        }

        $activities = PeepSoActivity::get_instance();

        $notif_act = $activities->get_activity_data($data['not_external_id'], $data['not_module_id']);
        $notif_external_id = 'user_comment' === $type ? $notif_act->act_comment_object_id : $notif_act->act_external_id;
        $notif_module_id = 'user_comment' === $type ? $notif_act->act_comment_module_id : $notif_act->act_module_id;

        $photo_code = FALSE;

        if ($notif_module_id == self::MODULE_ID) {
            $photo_act = $activities->get_activity_data($notif_external_id, $notif_module_id);
            $photo_post = $activities->get_activity_post($photo_act->act_id);

            // Same value between `$photo_act->act_external_id` and `$photo_post->ID`
            // indicates that user is commenting the post/album instead of a single photo item,
            // so we have to exclude it.
            if ( is_object($photo_post) && $photo_post->ID != $photo_act->act_external_id ) {
                $photo_code = 'photo=' . $notif_external_id;
            }
        }

        if ($photo_code) {
            $link .= (strpos($link, '#') ? '&' : '#') . $photo_code;
        }
