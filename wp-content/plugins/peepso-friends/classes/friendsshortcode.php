...
class PeepSoFriendsShortcode
{
	...
	public function show_friend($friend)
	{
        ...
        if( $friend->get_id() == get_current_user_id() ) {
                  $common_interests = '';
                } else {
                  $common_interests = '<a class="cus_btn_anchor" 
           
           href="javascript:show_interest('.$friend->get_id().')">
		...
		sprintf('<a class="ps-avatar ps-avatar--member" href="' . $friend->get_profileurl() . '"><img alt="%s avatar" title="%s" src="%s">' . $online . '</a>',
			...
			<div class="ps-member__cover" style="background-image:url(' . $friend->get_cover() . ')"></div>','
