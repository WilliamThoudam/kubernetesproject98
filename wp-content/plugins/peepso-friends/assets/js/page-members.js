...
$(function () {
	...
	$container.on('click', '.ps-js-friend-follow, .ps-js-friend-unfollow', function (e) {
		let $btn = $(e.currentTarget),
			$loading = $btn.find('img'),
			userId = $btn.data('user-id'),
			follow = $btn.hasClass('ps-js-friend-follow');
		...
		if (follow) {
			$loading.show();
			friends.follow(userId).then(function () {
				$loading.hide();
			});
		} else {
			$loading.show();
			friends.unfollow(userId).then(function () {
				$loading.hide();
			});
		}
	});
	$container.on('mouseenter', '.ps-js-friend-follow, .ps-js-friend-unfollow', function (e) {
		let $btn = $(e.currentTarget),
			$label = $btn.find('span').last();
	...
	$container.on('mouseleave', '.ps-js-friend-follow, .ps-js-friend-unfollow', function (e) {
		let $btn = $(e.currentTarget),
			$label = $btn.find('span').last();
	...
	hooks.addAction('friend_followed', 'page_members', updateActions);
	hooks.addAction('friend_unfollowed', 'page_members', updateActions);
