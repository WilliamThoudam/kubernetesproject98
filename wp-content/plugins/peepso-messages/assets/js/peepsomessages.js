...
(function ($, peepso, factory) {
	window.ps_messages = peepso.messages = factory($, peepso);
		...
		return $.Deferred(
			$.proxy(function (defer) {
				...
				peepso
					.disableAuth()
					.disableError()
					.postJson('messagesajax.get_messages_in_conversation', req, function (
						response
					) {
	...
	PsMessages.prototype.load_messages = function (type, page, query) {
		...
		peepso.postJson('messagesajax.get_messages', req, function (response) {
			...
			if (response.success) {
				...
				// Filter content.
				$messages = peepso.observer.applyFilters('messages_render', $messages);
	...
	PsMessages.prototype.confirm_mute_conversation = function (msg_id, elem) {
		...
		peepso.postJson('messagesajax.set_mute', params, response => {
			if (response.success) {
				peepso.observer.doAction('psmessages_conversation_mute', msg_id);
			}
		});
	...
	PsMessages.prototype.unmute_conversation = function (msg_id) {
		...
		peepso.postJson('messagesajax.set_mute', params, response => {
			if (response.success) {
				peepso.observer.doAction('psmessages_conversation_unmute', msg_id);
			}
		});
	...
	$(function () {
		...
		var elem = $('.ps-js-messages-notification')
			...
			.on('notifications.shown', function (e, obj) {
				...
				peepso.observer.applyFilters('messages_render', obj.popover_list.find(selector));
