...
class PeepSoMessages
{
	 ...
	 */
	 public function display_participant_summary()
 	{
 		...
 		$content_post = get_post($root);

         $grp_id = $content_post->post_content_filtered;
 		...
 		$participants_link = '<a href="#" onclick="ps_messages.show_long_participants(); return false;"> and '. ($num_participants-1) . _n(' other', ' others', $num_participants, 'msgso') . '</a>';
 		...
 		// Checking for message is in channel or normal
	...
	public function get_message_avatar($args)
	{
        ...
        $msg_id = intval($args['post_id']);
        $par_msg_id = wp_get_post_parent_id($msg_id);

        $content_post = get_post($par_msg_id);
		...
		if ($current_user_id === intval($args['post_author'])) {
			...
			if (count($participants) > 2) {
                ...
                if($content_post->post_content_filtered)
                {
                    $avatars[] = $coverUrl;
                }
                else
	...
	public function get_last_author_name($args)
	{
		...
		$par_msg_id = wp_get_post_parent_id($message_id);
		...
		if($grp_id != 0)
		{
			$in_conversation = count($participants);
		}
		else
		{
			$in_conversation = count($participants) -1;
