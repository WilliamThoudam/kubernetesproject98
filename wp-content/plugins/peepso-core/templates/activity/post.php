...
if ($slug == 'group')
{
    $group_id = get_post_meta($ID,'peepso_group_id', TRUE);
    $group = new PeepSoGroup($group_id);
    $parent_id = wp_get_post_parent_id($group->get('id'));
}
/*
$group_id = get_post_meta($ID,'peepso_group_id', TRUE);
...
//if($parent_id !=0){echo ('riyaz_post'); }

?>

<div class=" <?php if($parent_id !=0){echo ('riyaj_ps_post'); } ?> ps-post ps-js-activity <?php echo (TRUE == $pinned) ? 'ps-post--pinned ps-js-activity-pinned' : ''?> ps-js-activity--<?php echo $act_id; ?>"
	...
	if( PeepSo::is_admin() && TRUE == $pinned && !in_array($act_access, array(PeepSo::ACCESS_MEMBERS, PeepSo::ACCESS_PUBLIC)) ) {
		...
		echo '<div class="ps-post__warning">',__('This pinned post will not display to all users because of its privacy settings.', 'peepso-core'),'</div>';
	}
	?>
  ...
  <div class="ps-post__body ps-js-post-body">
		...
		<div class="ps-post__content ps-js-activity-content ps-js-activity-content--<?php echo $act_id; ?>"><?php $PeepSoActivity->content(); ?></div>
	  ...
	  <?php } ?>
		...
		<div class="ps-comments">
			...
			<div class="ps-comments__inner cstream-respond wall-cocs" id="wall-cmt-<?php echo $act_id; ?>">
				<div class=" <?php if($parent_id !=0){echo ('riyaj_ps-comments__list'); } ?> ps-comments__list comment-container ps-js-comment-container ps-js-comment-container--<?php echo $act_id; ?>"
		        ...
		        <div class="ps-comments__closed">
		            <i class="fas fa-lock"></i> <?php echo __('Commments are closed', 'peepso-core');?>
		        </div>
		        <?php }  ?>
				...
				<div id="act-new-comment-<?php echo $act_id; ?>" class="ps-comments__reply cstream-form stream-form wallform ps-js-comment-new ps-js-newcomment-<?php echo $act_id; ?>"
					...
					<div class="ps-comments__reply-send cstream-form-submit" style="display:none;">
						...
						<div class="ps-comments__reply-actions ps-comment-actions" style="display:none;">
							...
							<button onclick="return activity.comment_save(<?php echo $act_id; ?>, this);" class="ps-btn ps-btn--action ps-btn-primary ps-button-action" disabled><i class="dojokopost"></i></button>
						</div>
					</div>
				</div>
