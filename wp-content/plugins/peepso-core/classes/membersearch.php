...
class PeepSoMemberSearch extends PeepSoAjaxCallback
{
    ...
    public function show_member($member)
    {
        ...
        echo '<div class="ps-member__header">
                <a class="ps-avatar ps-avatar--member" href="' . $member->get_profileurl() . '">
					<img alt="' . strip_tags($member->get_fullname()) . ' avatar"
                    src="' . $member->get_avatar() . '">'.$online.'</a>
                <div class="ps-member__cover" style="background-image:url(' . $member->get_cover() . ')"></div>
          <div class="ps-member__options">';

          $this->member_options($member->get_id());
          
          if( $member->get_id() == get_current_user_id() ) {
                  $common_interests = '';
                } else {
                  $common_interests = '<a class="cus_btn_anchor" 
           
           href="javascript:show_interest('.$member->get_id().')">
           <img class="thunder_button_icon" src="https://s3.ap-south-1.amazonaws.com/cdn.dojoko.com/assets/magnet.svg">
           <span class="cus_cum_text">Common Interests</span>
           </a>';
                }
          ...
          <div class="ps-member__name">
  				<a href="' , $member->get_profileurl(), '" class="" title="', strip_tags($member->get_fullname()), '" alt="', strip_tags($member->get_fullname()), ' avatar">'
          , do_action('peepso_action_render_user_name_before', $member->get_id()), $member->get_fullname() , do_action('peepso_action_render_user_name_after', $member->get_id()) ,
          '</a>
          </div>
          ...
          $this->member_buttons_extra($member->get_id());

          echo '</div>';

        echo '</div>'; // close .ps-member__inner div
