...
class PeepSoProfile extends PeepSoAjaxCallback
{
    ...
    public function upload_avatar(PeepSoAjaxResponse $resp)
    {
        ...
        if($this->can_edit() && wp_verify_nonce($this->_input->value('_wpnonce','',FALSE), 'profile-avatar')) 
        {
            ...
            $checkResponse = get_user_meta($user_id, 'peepso_avatar_s3_host', TRUE);

            if($enable_aws_s3 && $checkResponse != null)
		    {
                $s3_token = get_user_meta($user_id, 'peepso_avatar_s3_host', TRUE);
		        $avatar_hash = get_user_meta($user_id, 'peepso_avatar_hash', TRUE). "-";
                $image_url = "https://" . $s3_token . "/peepso/users/". $user_id . "/" . $avatar_hash . 'avatar' . "-full" . '.jpg';
                $orig_image_url = "https://" . $s3_token . "/peepso/users/". $user_id . "/" . $avatar_hash . 'avatar' . "-orig" . '.jpg';

                // check image dimensions
                $si = new PeepSoSimpleImage();
                $orig_image_path = "https://" . $s3_token . "/peepso/users/". $user_id . "/" . $avatar_hash . 'avatar' . "-orig" . '.jpg';
                $si->load($orig_image_path);
                $width = $si->getWidth();
                $height = $si->getHeight();
                $avatar_size = PeepSo::get_option('avatar_size','250');
                if (($width < $avatar_size) || ($height < $avatar_size)) {
                    delete_user_meta($user_id, 'peepso_avatar_s3_host');
                ...
                $resp->set('orig_image_path', $image_url);
    ...
    public function crop(PeepSoAjaxResponse $resp)
    {
        ...
        // SQL safe, WP sanitizes it
        if($this->can_edit() && wp_verify_nonce($this->_input->value('_wpnonce','',FALSE), 'profile-avatar') && $this->can_edit()) {
            ...
            $preview = $this->_input->int('preview');
            ...
            if($enable_aws_s3 && (isset($checkResponse) && $checkResponse != null))
		    {
                $s3_token = get_user_meta($user_id, 'peepso_avatar_s3_host', TRUE);
                $avatar_hash = get_user_meta($user_id, 'peepso_avatar_hash', TRUE). "-";
		        $main_src_file = "https://" . $s3_token . "/peepso/users/". $user_id . "/" . $avatar_hash  . 'avatar-orig' . '.jpg';
                $src_file = "/tmp/". $avatar_hash  . 'avatar-orig' . '.jpg';
                $dest_file = "/tmp/" . $avatar_hash . 'avatar-full.jpg';
                $saved = copy($main_src_file, $src_file);
            }
            else
            {
                $src_file = $this->user->get_image_dir() . $avatar_hash  . 'avatar-orig' . ($tmp ? '-tmp' : '') . '.jpg';
                $dest_file = $this->user->get_image_dir() . $avatar_hash  . 'avatar-full' . ($tmp ? '-tmp' : ($preview ? '-preview' : '')) . '.jpg';
            }
            ...
            if($enable_aws_s3 && (isset($checkResponse) && $checkResponse != null))
		    {
                $avatar_hash = get_user_meta($user_id, 'peepso_avatar_hash', TRUE). "-";
                $src_file = "/tmp/". $avatar_hash  . 'avatar-orig' . '.jpg';
                $dest_file = "/tmp/" . $avatar_hash . 'avatar.jpg';
            }
            else
            {
                // re-crop thumbnailavatar image
                $dest_file = $this->user->get_image_dir() . $avatar_hash . 'avatar' . ($tmp ? '-tmp' : ($preview ? '-preview' : '')) . '.jpg';
            }
            ...
            if($enable_aws_s3 && (isset($checkResponse) && $checkResponse != null))
            {
                $dir = $this->user->get_image_dir();
                $avatar_hash = get_user_meta($user_id, 'peepso_avatar_hash', TRUE). "-";
                $dest_full = "/tmp/" . $avatar_hash . 'avatar-full.jpg';
                $dest_thumb = "/tmp/" . $avatar_hash . 'avatar.jpg';
                $file_full = $dir . $avatar_hash . 'avatar-full.jpg';
				$file_thumb = $dir . $avatar_hash . 'avatar.jpg';
                $avatar_dest = array($dest_full, $dest_thumb);
                $avatar_file = array($file_full, $file_thumb);
                
                foreach($avatar_dest as $key => $filepath)
				{
                    $filename = str_replace('\\', '/', 'peepso/' . substr($avatar_file[$key], strlen(PeepSo::get_peepso_dir())));
					$pho_token = $this->user->upload_to_amazon_s3($filepath, $filename, 'thumbnail');
					if (NULL !== $pho_token) 
					{
						$photo_data[] = $pho_token;
				    }
                }

                if(!empty($photo_data))
                {
                    $image_url = $this->user->get_avatar();
                    $resp->set('image_url', $image_url);
                    $resp->success(TRUE);
                    return;
                }
            ...
            else
            {
                if ($tmp) {
                    ...
                    $image_url = str_replace('.jpg', '-preview.jpg', $this->user->get_avatar());
                } else {
                    $image_url = $this->user->get_avatar();
                }
...
//        }
    ...
    public function interactions()
    {
        ...
        $views = TRUE;
        $privacy = PeepSo::get_option_new('profile_view_count_privacy_default');

        // Profile Views - Disabled Completely
        if( 99 == $privacy) {
            $views = FALSE;
        } elseif(!PeepSo::is_admin()) {
    ...
    public function get_alerts_definition($all = FALSE)
    {
        ...
        $activity_items = array(
            array(
                ...
                'descript' => __('Applies to all posts you wrote or commented on', 'peepso-core'),
    ...
    public function request_data_form()
    {
        ...
        if($can_edit) {
            ...
            $fields['submit'] = array(
                ...
                'class' => 'ps-btn--sm ps-btn--action ps-js-export-data-request cus_ps_js_export_data_request',
                'type' => 'submit',
            );
    ...
    public function render_preferences_form_field($field, $name)
    {
        ...
        foreach ($field['fields'] as $value) {
            ...
            if ('checkbox' === $value['type']) {
                ...
                $custom_field .= "<div class='ps-profile__notification-title' id='".$value['id']."'>".$value['label']."</div>";
            $custom_field .= '</span>';
        }
