...
class PeepSoUser
{
    ...
    public function get_cover($size=0)
    {
		...
		if($enable_aws_s3 && $cover_hash != null)
		{
			global $wpdb;
			$table = $wpdb->prefix . self::TABLE;
			$user_id = $this->id;
			$query = 'Select * from '.$table.' where usr_id = '.$user_id;
			$cover = $wpdb->get_row($wpdb->prepare($query))->usr_cover_photo;
		}
		else
		{
			$filename = $cover_hash . 'cover.jpg';
	...
	public function get_avatar($suffix ='full')
	{
		...
		$user_id = $this->get_id();
		$s3_token = get_user_meta($this->get_id(),'peepso_avatar_s3_host', TRUE);
		...
		$enable_aws_s3 = PeepSo::get_option('photos_enable_aws_s3');
		if($enable_aws_s3 && (isset($s3_token) && $s3_token != null))
		{
			$file = "https://" . $s3_token . "/peepso/users/". $user_id . "/" . $avatar_hash . 'avatar' . $suffix . '.jpg';
		}
		else
		{
    ...
    public function get_default_avatar($full = TRUE) {
		...
		return PeepSo::get_asset('images/avatar/user-' . $gender . ($full ? '' : '-thumb') . '.png');
    }
...
//		$content = PeepSoMailQueue::add_message($this->id, $data, sprintf(__('Welcome to %s', 'peepso-core'), get_bloginfo('sitename')), 'new_user', 'new_user');
	...
	public function move_cover_file($src_file, $add_to_stream=TRUE)
	{
		...
		if($enable_aws_s3)
		{
			...
			$filename = str_replace('\\', '/', 'peepso/' . substr($dest_file, strlen(PeepSo::get_peepso_dir())));

			$pho_token = $this->upload_to_amazon_s3($dest_file, $filename, 'cover');
			if(NULL !== $pho_token)
			{
				// update database table entry
				$data = array('usr_cover_photo' => $pho_token);
	...
	public function delete_avatar()
	{
		...
		$meta = $this->check_meta_data($this->get_id());
		if($meta == true) {
			delete_user_meta($this->get_id(), 'peepso_avatar_s3_host');
		}
	...
	public function move_avatar_file($src_file, $delete = FALSE)
	{
		...
		if (!is_wp_error($image)) {
			...
			if($enable_aws_s3)
			{
				...
				$file_orig = $dir . $avatar_hash . '-avatar-orig.jpg';
				$file_full = $dir . $avatar_hash . '-avatar-full.jpg';
				$file_thumb = $dir . $avatar_hash . '-avatar.jpg';
				$dest_orig = "/tmp/" . $avatar_hash . '-avatar-orig.jpg';
				...
				$dest_orig = $image->save($dest_orig, IMAGETYPE_JPEG);
	
				$image = wp_get_image_editor($src_file);
				$image->resize(PeepSo::get_option('avatar_size', 250), PeepSo::get_option('avatar_size', 250), TRUE);
				$image->set_quality(PeepSo::get_option('avatar_quality', 75));
				$image = $this->fix_image_orientation($image, $orientation);
				$dest_full = $image->save($dest_full, IMAGETYPE_JPEG);
	
				$image = wp_get_image_editor($src_file);
				$image->resize(self::THUMB_WIDTH, self::THUMB_WIDTH, TRUE);
				$image->set_quality(PeepSo::get_option('avatar_quality', 75));
				$image = $this->fix_image_orientation($image, $orientation);
				$dest_thumb = $image->save($dest_thumb, IMAGETYPE_JPEG);
				
				$avatar_file_path = array($dest_orig, $dest_full, $dest_thumb);
				$avatar_file = array($file_orig, $file_full, $file_thumb);
				
				foreach($avatar_file_path as $key => $filepath)
				{
					$filename = str_replace('\\', '/', 'peepso/' . substr($avatar_file[$key], strlen(PeepSo::get_peepso_dir())));
					$pho_token = $this->upload_to_amazon_s3($filepath['path'], $filename, 'avatar');
					if (NULL !== $pho_token) 
					{
	 ...
	 */
	public function upload_to_amazon_s3($filepath, $filename, $filetype)
	{
		$photo_model = new PeepSoPhotosModel();
		$s3 = $photo_model->get_amazon_s3();
		if (NULL === $s3) {
			return (NULL);
		}
		...
		try 
		{
			...
			{
				return 'There was a problem when uploading the file';
			} 
			else 
			{
				if($filetype == 'avatar')
				{
					$s3_url = $metadata['effectiveUri'];
					$parse = parse_url($s3_url);
					$host = $parse['host'];
					$s3_host = str_ireplace('www.', '', $host);
					$meta = $this->check_meta_data($this->get_id());
					if($meta == true)
					{
						update_user_meta($this->get_id(),'peepso_avatar_s3_host', $s3_host);
					}
					else
					{
						add_user_meta($this->get_id(),'peepso_avatar_s3_host', $s3_host);
					}
	...
	public function check_meta_data($id)
	{
		$checkResponse = get_user_meta($id,'peepso_avatar_s3_host', TRUE);
		if(isset($checkResponse) && $checkResponse != null)
		{
			return true;
		}
		else
	...
	/* Ending Of Customized Code
	 * AWS s3 Bucket Uploading avatar image copied from photosmodel.php - 
	 * Peepso Photos Plugin code
	 * /
