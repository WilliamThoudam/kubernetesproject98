...
//        $wp_query->is_page = FALSE;
    ...
    public function retrieve_password()
    {
        ...
        $email = $input->value('email','',false); // SQL safe, is_email

        if (empty($email)) {
            $errors->add('empty_username', __('Please enter your email address.', 'peepso-core'));
        } else if (is_email($email)) { 
            $user_data = get_user_by('email', sanitize_email($email));
            if (empty($user_data))
                $errors->add('invalid_email', __('Try again. No such user exists.   ', 'peepso-core'));
        }
