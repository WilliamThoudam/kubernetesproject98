...
//		$wp_query->is_page = FALSE;
	...
	private function register_user()
	{
		...
		if (wp_verify_nonce($sNonce, 'register-form')) {
            ...
            $grpid = $input->value('group_id', '', FALSE); // SQL Safe
            $sid = $input->value('sender_id', '', FALSE); // SQL Safe
            $n_user_email = $input->value('nuser_email_id', '', FALSE); // SQL Safe


            if(($grpid != 0) && ($sid != 0)  && (($n_user_email == 'none') || ($n_user_email == $email)))
            {

             /*if(isset($n_user_email) || $n_user_email == $email)
               {
             */   global $wpdb;

                $table_name = 'wp_peepso_group_members';

                $sql1 = $wpdb->prepare("INSERT INTO ".$table_name."(gm_user_id,gm_group_id,gm_user_status,gm_invited_by_id) VALUES ( %d, %d, %s, %d)",$wpuser,$grpid,'member',$sid);

                $wpdb->query($sql1);

                $table_name_not = 'wp_peepso_notifications';

                $sql2 = $wpdb->prepare( "INSERT INTO ".$table_name_not." (not_user_id,not_from_user_id,not_module_id,not_external_id,not_type,not_message) VALUES
                ( %d, %d, %d, %d, %s, %s)",
                 $wpuser,
                 $sid,
                 8,
	...
	public function activate_account()
	{
        ...
        if (count($user->results) > 0) {
            ...
            if('ban' == $wpuser->get_user_role()) {
                echo __('Your account has been suspended.', 'peepso-core');
                return;
            }
