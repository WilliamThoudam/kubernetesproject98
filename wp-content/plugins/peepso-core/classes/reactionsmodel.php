...
class PeepSoReactionsModel
{
	...
	public function __construct($all = TRUE)
	{
		...
		foreach($posts->posts as $post) {
			...
			$this->reactions[$reaction['id']] = (object) $reaction;
		}
	}
	...
	public function user_reaction_set( $react_id )
	{
        ...
        new PeepSo3_Activity_Notifications($post_id, $owner_id);
		...
		if( 0 == $react_id ) {
			...
			if( TRUE === $do_notify ) {
				// send LIKE email
				PeepSoMailQueue::add_notification($owner_id, $mailq_data, sprintf(__('Someone liked your %s', 'peepso-core'), $activity_type), 'like_post', 'like_post', PeepSoActivity::MODULE_ID);

				// add LIKE notification
				$notification_message = sprintf(__('liked your %s', 'peepso-core'), $post_type_object->labels->activity_type);
				$note->add_notification($this->_user_id, $owner_id, $notification_message, 'like_post', $this->act_module_id, $post_id);
			}
			return TRUE;
		}
		...
		if( TRUE == $do_notify ) {
			// send REACT email
			$message_title = __('Someone', 'peepso-core')
				. ' ' . $this->reaction($react_id)->content
				. ' ' . sprintf(__('your %s', 'peepso-core'), $activity_type);

			PeepSoMailQueue::add_notification($owner_id, $mailq_data, $message_title, 'like_post', 'like_post', PeepSoActivity::MODULE_ID);

			// add REACT notification
			$notification_message = $this->reaction($react_id)->content . ' ' . sprintf(__('your %s', 'peepso-core'), $post_type_object->labels->activity_type);
			$note->add_notification($this->_user_id, $owner_id, $notification_message, 'like_post', $this->act_module_id, $post_id);
		}
			...
			<div class="ps-reactions__list">
				...
				foreach ($this->reactions as $react_id => $reaction) :
					...
					<span class="ps-reactions__list-item">
						<a aria-label="<?php echo $this->reaction($react_id)->title;?>" class="<?php echo $class;?>" href="#"
						   data-tooltip="<?php echo $this->reaction($react_id)->title; ?>"
