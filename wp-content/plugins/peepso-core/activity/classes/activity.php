...
//			'post_date_gmt' => date('Y-m-d H:i:s'), // gmdate('Y-m-d H:i:s'),
    ...
    public function add_comment($post_id, $author_id, $content, $extra = array())
    {
        ...
        new PeepSo3_Activity_Notifications($parent_post_id, $parent_post_owner_id);

        // activity_followers  for comment author
        new PeepSo3_Activity_Notifications($parent_post_id, $author_id);
    ...
    public function comment_actions()
    {
        ...
        if($show_replybutton) {
            ...
            $acts['reply'] = array(
                'href' => '#reply',
                'label' => $reply_label,
                'class' => 'ps-comment__action--reply actaction-reply',
                'icon' => $reply_icon,
        ...
        foreach ($acts as $name => $act) {
            ...
            echo ' class="ps-comment__action ', $act['class'], '">';
...
//        }
    ...
    public function post_actions()
    {
        ...
        //             'href' => '#',
        //             'label' => __('Remove Link Preview', 'peepso-core'),
        //             'class' => 'ps-post__action ps-post__action--preview actaction-removepreview',
        ...
        $acts['repost'] = array(
                ...
                'class' => 'cus_repost_btn ps-post__action',
                'icon' => 'gcis gci-share-square',
                'click' => 'return activity.action_repost('.$this->post_data['act_id'].', this);',
            );
        ...
        if ($logged_in && PeepSo::get_option_new('post_save_enable')) {
            $acts['save'] = array(
                ...
                'class' => 'ps-post__action ps-post__action--save actaction-save ps-js-save-toggle',
    ...
    public function post_options()
    {
        ...
        if(defined('PEEPSO_NSFW')) {

            $options['nsfw'] = array(
                'label' => __('Mark NSFW', 'peepso-core'),
                'icon' => 'gcis gci-exclamation-circle',
                'click' => 'return activity.action_nsfw(' . $post->ID . ', 1);',
            );

            if ($this->is_nsfw($post)) {
                $options['nsfw'] = array(
                    'label' => __('Not NSFW', 'peepso-core'),
                    'icon' => 'gcis gci-exclamation-circle', // 'remove',
                    'click' => 'return activity.action_nsfw(' . $post->ID . ', 0);',
                );
            }
...
//        }
        ...
        if (PeepSo::can_pin(intval($post->ID))) {
            $options['pin'] = array(
                ...
                'icon' => 'gcis gci-sort-amount-up', // 'remove',
            ...
            if ($this->is_pinned($post)) {
                $options['unpin'] = array(
                    ...
                    'icon' => 'gcis gci-sort-amount-down-alt', // 'remove',
        ...
        foreach ($options as $name => $data) {
            if($data['label'] !='RePost')
            {
                ...
                if (isset($data['extra']))
                    echo ' ', $data['extra'];
                if (isset($data['click']))
                    echo ' onclick="' . rtrim( esc_js($data['click']), ';' ) . '; return false"';
    ...
    public function show_post($no_html = FALSE)
    {
        ...
        $this->post_data['slug'] = $this->_input->value('context','',FALSE);
        ...
        // Refresh media
        $refresh = PeepSo::get_option('refresh_embeds');
        if( $refresh > 0 && !strlen(PeepSo3_Mayfly::get($mayfly = 'peepso_cache_media_'.$this->post_data['ID']))) {
    ...
    public function nsfw(PeepSoAjaxResponse $resp)
    {
        ...
        // BREAK If not admin
        // @TODO for now we are using the can_pin permission
        if (!PeepSo::can_pin($post->ID)) {
            $resp->error(__('You do not have permission to do that', 'peepso-core'));
            $resp->success(FALSE);
            return( FALSE );
    ...
    public function post_action_title()
    {
        ...
        if ($post->post_author === $post->act_owner_id) {
            ...
            $title = sprintf(
                '<a class="ps-post__author" href="%s" data-hover-card="%d">%s</a><i class="gcis gci-angle-right"></i><a class="ps-post__subtitle" href="%s" data-hover-card="%d">%s</a>',
