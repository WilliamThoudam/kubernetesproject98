    ...
    private function __construct()
    {
        ...
        add_action('init', function(){
            ...
            if (NULL == $check_plugins) {
                ...
                if(strlen(PeepSo3_Mayfly::get('peepso_is_offline'))) {
        ...
        if (is_admin() && !PeepSo::is_api_request()) {
            ...
            // Bundle
            add_action('admin_notices', function () {
                if (isset($_GET['peepso_hide_bundle'])) {
                    update_user_option(get_current_user_id(), 'peepso_hide_bundle', 1);
                }

                if (isset($_GET['peepso_hide_bundle_reset'])) {
                    delete_user_option(get_current_user_id(), 'peepso_hide_bundle');
                }

                // do nothing if user dismissed it
                if (get_user_option('peepso_hide_bundle')) {
                    return;
                }
...
//				$post = get_post($repost)
            ...
            add_filter('peepso_stream_id_list', function ($filters) {
                ...
                if(class_exists('PeepSoGroupsPlugin') || class_exists('PeepSoFriends')) {
                    ...
                    $filters['core_following'] = array(
                        ...
                        'icon' => 'gcis gci-check-circle ps-js-icon',
                    );
                }
                ...
                if(PeepSo::can_schedule_posts()) {
                    ...
                    $filters['core_scheduled'] = array(
                        ...
                        'desc' =>   $desc,
                        'icon'=> 'gcis gci-clock ps-js-icon',
                    );
                }
    ...
    public function check_query()
    {
        ...
        if (isset($_GET['logout'])) {
            ...
            setcookie('peepso_last_visited_page', $current_page, time() + (MINUTE_IN_SECONDS * 30), '/', null, isset($_SERVER['HTTPS']) ? true : false, true);
...
//		}
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_script('peepso-widget-search', PeepSo::get_asset('js/widgets/widget-search.js'), array('jquery'), PeepSo::PLUGIN_VERSION, TRUE);
        wp_enqueue_script('peepso-bundle', PeepSo::get_asset('js/bundle.min.js'),
            array('jquery-ui-position', 'peepso', 'peepso-datepicker', 'peepso-window', 'peepso-avatar-dialog', 'peepso-widget-search'), PeepSo::PLUGIN_VERSION, TRUE);
    ...
    public static function get_page($name)
    {
        switch ($name) {
            ...
            case 'activation_redirect':
                ...
                if(-1 == $page_id) {
                    if (isset($_SESSION['peepso_first_visit']) && !empty($_SESSION['peepso_first_visit'])) {
                        $ret = $_SESSION['peepso_first_visit'];
                    } else {
                        $ret = home_url('/');
                    }
                }elseif (0 == $page_id) {
    ...
    */
    ...
    public function filter_peepso_navigation_profile_final($links)
    {
        foreach($links as $key=>$link) {
            if(is_array($link) && $key != $link['href'] && $key!='stream') {
                trigger_error("peepso_navigation_profile: {$link['href']} *href* should be the same as *id* ($key)");
            }
        }


        $order = apply_filters('peepso_filter_navigation_profile_order', ['blogposts','friends','photos','media','videos','audio','groups']);

        // Stream ,About, CV are always on top, Edited by Rishbah
        $order = array_merge(array('stream', 'about', 'cv'), $order);
    ...
    public function add_fieldtext_admin_general_option($field) {
        ...
        if($field->prop('admin_registration_only')) {
	        $params = array(
		        ...
		        'field'       => $field,
		        'label'       => __( 'Show in only registration', 'peepso-core' ),
		        'label_after' => __( 'If enabled, the field will only show during registration and will not be counted in profile completeness', 'peepso-core' ),
	        );
