...
class PeepSo3_Mayfly {
	...
	public static function set(string $name, $value, int $ttl = -1) {
		...
		if($sanitized_name != $name) {
			trigger_error('mayfly::'.__FUNCTION__ . ': Name must be a combination of letters, underscores and numbers - ' . maybe_serialize($name), E_USER_ERROR);
		}
		...
		if( !is_string($value) && !is_numeric($value) && !is_object($value)  && !is_array($value) ) {
			...
			echo "</pre>";
			trigger_error( 'mayfly::'.__FUNCTION__ .': Value must be numeric, string, array or object - ' . maybe_serialize($value), E_USER_ERROR);
		}
