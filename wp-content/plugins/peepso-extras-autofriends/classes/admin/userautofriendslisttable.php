...
//			$order = ($order === 'ASC') ? 'ASC' : 'DESC';
	...
	public function get_columns()
	{
		$columns = array(
			...
			'fullname' => __('Name', 'autofriends-peepso'),
			'friends' => __('Friends', 'autofriends-peepso'),
			'actions' => __('Actions', 'autofriends-peepso')
		);
	...
	public function get_column_data($item, $column_name)
	{
		switch ($column_name)
		{
			...
			case 'friends':
				$friends = new PeepSoFriendsModel();
				return $friends->get_num_friends($item['af_user_id']);
				break;
	...
	public function column_actions($item)
	{
		$actions = array(
            'befriend'   => '<a href="#" onclick="list.befriend(' . $item['af_user_id'] . '); return false;">
            	<input type="button" value="' . __('Befriend All Users', 'autofriends-peepso') . '" ' .
				' title="' . __('Befriend All users.', 'autofriends-peepso') . '" class="button action"/></a>',

			'remove'   => '<a href="#" onclick="list.remove(' . $item['af_user_id'] . '); return false;">
            	<input type="button" value="' . __('Remove', 'autofriends-peepso') . '" ' .
				' title="' . __('Remove from list autofriends creation.', 'autofriends-peepso') . '" class="button action"/></a>',
        );
	...
	{
		return (array(
			'befriend' => __('Befriend All Users', 'autofriends-peepso'),
			'remove' => __('Remove', 'autofriends-peepso')
		));
	}
	...
	public function process_bulk_action()
	{
		if ($this->current_action() && check_admin_referer('bulk-action', 'autofriends-nonce')) {
			...
			if ('befriend' === $this->current_action()) {
				...
				$message = __('befriend', 'autofriends-peepso');
			} else if ('remove' === $this->current_action()) {
				foreach ($_POST['users'] as $userId){
					$count += $oAutofriends->remove_user(intval($userId));
				}

				$message = __('removed', 'autofriends-peepso');
			}

			PeepSoAdmin::get_instance()->add_notice(
				sprintf('%1$d %2$s %3$s',
					$count,
					_n('user', 'users', $count, 'autofriends-peepso'),
					$message),
				'note');
